...
output:
  knitrBootstrap::bootstrap_document:
    title: "Compare translation levels between species"
    theme: default
    theme.chooser: TRUE
...
```{r, eval = FALSE}
# Make expression data set from read count table
source(file.path(codedir,"makeExpressionSet_ribo.r"))
riboCountFiles <- list.files(file.path(dir,"data/ribo"),
                             pattern="*_081514.table",full.name=TRUE)
makeSet <- makeExpressionSet_ribo(riboCountFiles)
eSet.ribo <- makeSet$eSet

# Remove reference cell line from the expression set
eSet.ribo.none <- eSet.ribo[,eSet.ribo$celline!="H19238"]

# Filtering: include only genes quantified in 3 or more samples 
...
# Compute RPKM values
ribo.rpkm <- 100*( t(t(exprs(eSet.ribo.none.f) )/
                colSums(exprs(eSet.ribo.none.f)))*(10^6))/
                fData(eSet.ribo.none.f)$len

# Replace the zero RPKM with NA's
ii.zero <- which(ribo.rpkm == 0,arr.ind=TRUE)
for (i in 1:dim(ii.zero)[1]) {
  ribo.rpkm[ii.zero[i,1], ii.zero[i,2]] = NA
}

# Qunatile normalize the RPKM values
source(file.path(codedir,"normalizationMethods.r"))
ribo.rpkm.nos.q <- normalize(ribo.rpkm,method="quantile",log=FALSE)

# Make an expression set of ribosome profiling RPKM values
# quantie-normalized across species
# Exclude the standard cell line
eSet.ribo.rpkm.nos.q <-
  ExpressionSet(assayData = as.matrix(log2(ribo.rpkm.nos.q)),
...
# Compute relative RPKM 
# Only keep genes quantified with more than 1 read in the 
# internal standard cell line
...
# on the relative RPKM values
source(file.path(codedir,"DEtesting.r"))
ribo.Res.hc <- testDE(eSet.ribo.rpkmq,seqData="ribo",
                      species=c("human","chimp"))$res
ribo.Res.hr <- testDE(eSet.ribo.rpkmq,seqData="ribo",
                      species=c("human","rhesus"))$res
ribo.Res.rc <- testDE(eSet.ribo.rpkmq,seqData="ribo",
                      species=c("chimp","rhesus"))$res

# Differential expression testing on 
# the RPKM values
source(file.path(codedir,"DEtesting.r"))
ribo.Res.hc.nos <- testDE(eSet.ribo.rpkm.nos.q,seqData="ribo",
                          species=c("human","chimp"))$res
ribo.Res.hr.nos = testDE(eSet.ribo.rpkm.nos.q,seqData="ribo",
                         species=c("human","rhesus"))$res
ribo.Res.rc.nos = testDE(eSet.ribo.rpkm.nos.q,seqData="ribo",
                         species=c("chimp","rhesus"))$res
...
boxplot(log2(ribo.rpkm),
        ...
        outcol=c(rep("blue",5),rep("red",5),rep("green",5)),
...
boxplot(log2(ribo.rpkm.s),
        ...
        outcol=c(rep("blue",5),rep("red",6),rep("green",5)),
        axes=F,main="",xlab="",ylab="")
...
```{r, eval=F,fig.height=4,fig.width=4,include = FALSE}
