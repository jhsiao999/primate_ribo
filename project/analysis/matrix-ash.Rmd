...
```{r}
post_means <- read.table(file = "~/Dropbox/joyce (1)/jul30posterior.means.txt")[,-1]

load(file.path(rdadir, "Mash.rda"))

all.dat <- apply(dmat_unnormed, 2, function(x) {
                  qqnorm(x,plot.it = F)$x
                  })
par(mfrow = c(2,2))
plot(all.dat[,1],post_means[,1],main="PosteriorMeans vs Z stat in Ribosome",
     xlim = c(-3, 3), ylim = c(-3, 3))
abline(0, 1, col = "red")


plot(all.dat[,2],post_means[,2],main="PosteriorMeans vs Z stat in RNA",
     xlim = c(-3, 3), ylim = c(-3, 3))
abline(0, 1, col = "red")

plot(all.dat[,3],post_means[,3],main="PosteriorMeans vs Z stat in Protein",
     xlim = c(-3, 3), ylim = c(-3, 3))
abline(0, 1, col = "red")
...
par(mfrow = c(2,2))
plot(all.dat[res.riborna$int.qval < .01, 1],
     post_means[res.riborna$int.qval < .01, 1],
     main="PosteriorMeans vs Z stat in Ribosome",
     xlim = c(-3, 3), ylim = c(-3, 3), col = "blue")
abline(0, 1, col = "red")

plot(all.dat[res.riborna$int.qval < .01, 2],
     post_means[res.riborna$int.qval < .01, 2],
     main="PosteriorMeans vs Z stat in RNA",
     xlim = c(-3, 3), ylim = c(-3, 3), col = "blue")
abline(0, 1, col = "red")

plot(all.dat[res.riborna$int.qval < .01, 3],
     post_means[res.riborna$int.qval < .01, 3],
     main="PosteriorMeans vs Z stat in protein",
     xlim = c(-3, 3), ylim = c(-3, 3), col = "blue")
abline(0, 1, col = "red")
