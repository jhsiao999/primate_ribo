...
[view source](translation-efficiency-protein-divergence-20150812.Rmd)
...
between human and chimpanzee. We then categorize the genes into
attenuation or enhancement at the translation level. Finally, we compute
the proportion of genes attenuated at the protein level in the set of genes
...
```{r}
no_samples <- sum(ii_sig_rnaribo)
prop.test(sum(ii_rnaGTribo), no_samples)
```
